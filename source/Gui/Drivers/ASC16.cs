...
namespace Oceano.Gui.Drivers
{
    public static class ASC16
    {
        ...
        public static void DrawStringVGA(this Canvas canvas, Color color, string s, int x, int y)
        {
            //ASCII
        ...
        public static void DrawStringSVGA(this VMWareSVGAII vMWareSVGAII, string s, uint color, uint x, uint y)
        {
            string[] lines = s.Split('\n');
            for (int l = 0; l < lines.Length; l++)
            {
                for (int c = 0; c < lines[l].Length; c++)
                {
                    ...
                    for (int i = 0; i < 16; i++)
                    {
                        for (int j = 0; j < 8; j++)
                        {
                            if ((fontbuf[i] & (0x80 >> j)) != 0)
                            {
                                vMWareSVGAII.SetPixel((uint)((x + j) + (c * 8)), (uint)(y + i + (l * 16)), color);
                            }
                        }
                    }
                }
        ...
        public static void DrawStringVBE(this PrismGraphics.Extentions.VBECanvas canvas, PrismGraphics.Color color, string s, int x, int y)
        {
            //ASCII
            string[] lines = s.Split('\n');
            for (int l = 0; l < lines.Length; l++)
