...
public class VMWareSVGAII
{
    ...
    public void Fill(uint x, uint y, uint width, uint height, uint color)
    {
        ...
        else
        {
            if ((capabilities & (uint)Capability.RectCopy) != 0)
            {
                ...
                for (var xTmp = x; xTmp < xTarget; xTmp++) SetPixel(xTmp, y, color);
                // refresh first line for copy process
                Update(x, y, width, 1);
                for (var yTmp = y + 1; yTmp < yTarget; yTmp++) Copy(x, y, x, yTmp, width, 1);
            }
            else
            {
                var xTarget = x + width;
    ...
    public void DefineAlphaCursor(Bitmap bitmap)
    {
        ...
        for (var i = 0; i < bitmap.RawData.Length; i++)
    ...
    public void DrawFilledRectangle(int x, int y, int width, int height, Color color)
    {
        ...
        for (var i = Math.Max(0, x); i < Math.Min(ReadRegister(Register.Width), x + width); i++)
        for (var j = Math.Max(0, y); j < Math.Min(ReadRegister(Register.Height), y + height); j++)
            Buffer[j * ReadRegister(Register.Width) + i] = argb;
    }
    ...
    public void DrawImageAlpha(Bitmap bitmap, int x, int y)
    {
        ...
        for (var j = 0; j < bitmap.Width; j++)
        {
            var from = GetPixel(x + j, y + i);
            var to = Color.FromArgb(bitmap.RawData[i * bitmap.Width + j]);
